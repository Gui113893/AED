...
void GraphDFSWithStackDisplay(const GraphDFSWithStack* p) {
  assert(p != NULL);
  // TO BE COMPLETED !!
  if (GraphIsDigraph(p->graph)) printf("O grafo é Direcional\n");
  else printf("O grafo NÃO é Direcional\n");

  if (GraphIsWeighted(p->graph)) printf("O grafo é Weighted\n");
  else printf("O grafo NÃO é Weighted\n");

  if (GraphIsComplete(p->graph)) printf("O grafo é Completo\n");
  else printf("O grafo NÃO é Completo");
}
